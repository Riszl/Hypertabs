    ...
    return async function onRequest(clientRequest, clientResponse) {
        try {
            ...
            ((requestContext.url.protocol == 'https:' || ctx.config.forceHttps) ? https : http).request({
                ...
                clientResponse.end(`<html><head><style>body {display:flex;justify-content:center;align-items:center;}img {margin-bottom:30px;display:block;}span {display:block;}</style></head><body><div><img src="/favicon.ico" class="error" height=100><span style="font-family:sans-serif;font-size:35px;font-weight:bolder;">This site can't be reached</span><span style="font-family:sans-serif;font-size:20px;font-weight:normal;">Check if there is a typo in ${new URL(ctx.url.unwrap(clientRequest.url, { flags: true, leftovers: true, }).value).hostname}</span></div></body></html>`);
